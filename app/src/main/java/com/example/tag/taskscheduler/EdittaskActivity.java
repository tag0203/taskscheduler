...
public class EdittaskActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final int initial_cutvalue = intent.getIntExtra("cut task", 0);
        ...
        for(int i = 0;i<=cuttime_list.size();i++){
            if(i < cuttime_list.size()){
                ...
                if(initial_cutvalue == 0)
                    cut_list.add(String.valueOf(i+1));
            }
        }
        spinner_adapter(cut_num,cut_list);

        final int initial_value = intent.getIntExtra("i", 0);
        input_taskname.setText(intent.getStringExtra("name"), TextView.BufferType.NORMAL);
        time.setText(String.valueOf(intent.getIntExtra("task time", 0)), TextView.BufferType.NORMAL);
        cutting_time.setText(String.valueOf(intent.getIntExtra("cut time", 0)), TextView.BufferType.NORMAL);
        num.setSelection(initial_value);
        cut_num.setSelection(initial_cutvalue);
        ...
        save.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(input_taskname.length() == 0){
                    Toast toast = Toast.makeText(EdittaskActivity.this, "タスク名が入力されていません", Toast.LENGTH_LONG);
                    toast.show();
                }else if(time.length() == 0){
                    Toast toast = Toast.makeText(EdittaskActivity.this, "タスクにかけたい時間が入力されていません", Toast.LENGTH_LONG);
                    toast.show();
                }else if(cutting_time.length() == 0){
                    Toast toast = Toast.makeText(EdittaskActivity.this, "最低でもタスクにかけたい時間が入力されていません", Toast.LENGTH_LONG);
                    toast.show();
                }else if(Integer.parseInt(time.getText().toString()) <= Integer.parseInt(cutting_time.getText().toString())){
                    Toast toast = Toast.makeText(EdittaskActivity.this, "タスクにかけたい時間が省略できる時間より短いです", Toast.LENGTH_LONG);
                    toast.show();
                }else{
                    ...
                    send.putExtra("initial value", initial_value);
